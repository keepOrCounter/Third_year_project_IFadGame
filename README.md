...
核心目标是解决一个工程问题：大模型天生会编造，怎么让它在一个有规则、有状态的游戏系统里老实干活。
...
*（以上完整摘自项目运行日志 `modules/log.txt`，是引擎实时生成的真实输出。
...
但立刻引入新问题：**大模型会产生幻觉，编出与之前游戏状态矛盾的情节**，比如之前的叙述中玩家背包里没有钥匙，
模型却写玩家「用钥匙打开了门」。

项目要验证的是：能不能靠架构设计，而不是靠堆提示词，把这个问题从根本上抑制住。

## 核心设计：模拟系统是唯一事实来源

- **一套不含任何 AI 的常规程序独占世界的全部事实**：玩家有多少体力、背包里有几块面包、这片森林里有没有狼，
  全部由代码精确记录和判定。
- **大模型只做一件事**：把这些既定事实写成好看的描述。
...
```mermaid
flowchart TD
    A["玩家输入英文指令，例如 go north"] --> B["理解玩家想做什么<br/>（拼写纠正 + 语法分析 + 模糊匹配）"]
    B -->|"看不懂"| Z["提示玩家换个说法"]
...
世界是可以无限生成，走到边界就现场生成下一块区域，最容易出的问题是玩家原路返回时看到一片完全不同的土地。
...
- **物品自动分布**：每件物品带有各地形下的出现概率，芦荟长在沙滩、鱼出现在河里，无需逐个地点手工编排
...
1. **上下文感知的提示词模板**：每次只把与当前场景相关的信息交给模型，并先做上面第二点的数值转人话处理。并通过样例以及提示词来约束大模型的输出。
2. **小样本微调**：人工编写「游戏状态 -> 理想叙事」样本，用 OpenAI 微调接口训练专用模型。实际用于微调的是
   `data/output.jsonl`，共 **16 条**。

**结果是两条路径都没能弥补 GPT-3.5 自身推理能力的差距。** 提示词工程能确定格式和语气，但无法完全抑制幻觉：
当保持一致需要模型跨好几项状态推演一条因果链时，GPT-3.5 依然会出错。微调这一侧的瓶颈很明确，
16 条样本数据量远远不够，模型没能学到稳定的模式。

这不是一个 完全成功 的实验，但把问题定位清楚比强行包装成功更有价值。如果未来重做，可能的做法是收集更多的数据或者更换更好的大模型。
...
需要 **Python 3.9+** 与一个 OpenAI API key。
...
失败的是测试代码本身而非游戏逻辑，原因见[已知限制](#已知限制)。
...
这是 2024 年 4 月提交时的代码，此后未做现代化改造。四类已知问题：

1. **GPT 模型配置使用了微调模型**：微调模型仅对训练账号可见，SDK 停留在 `openai<1.0`，
   底座模型也已退场。可能的修法是让模型、key 与 `base_url` 全部走环境变量。
2. **返回值解析不够健壮**：裸 `except`、重试耗尽后的未绑定变量、按键序取值、奖惩索引不校验。
   可能的修法是启用 JSON mode 加 schema 校验，并在重试耗尽时降级而不是崩溃。
3. **测试套件已漂移**：7 个用例中只有 1 个能通过，但失败的是测试代码本身，不是游戏逻辑。
   引擎在项目后期改过接口，测试没有同步：动作名从 `"Move"` 改成了 `"Go"`，`npcGenerator` 多了一个参数，
   `pickUp` 与 `equip` 现在要求传入完整的动作对象而测试仍传 `None`。都是改名和签名变更，游戏本身照常运行。
